...
class JibriManager : StatusPublisher<Any>() {
    ...
    private val xmppEnvironments = configSupplier<List<XmppEnvironmentConfig>> {
        ...
        "jibri.api.xmpp.environments".from(Config.configSource).convertFrom<List<com.typesafe.config.Config>> {
             envConfigs -> envConfigs.map { it.toXmppEnvironment() } }
    }.get()

    fun xmppServerHost(): String {
        return xmppEnvironments[0].xmppServerHosts.get(index = 0)
    }
    ...
    val gmeetPort: String by config {
        "JibriConfig::gmeetPort" { Config.legacyConfigSource.gmeetPort!! }
        "jibri.gmeet-port".from(Config.configSource)
    }
